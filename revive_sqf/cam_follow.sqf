...
while {!_destroy} do 
{
	...
	if (_c == 0 ) then
	{
		...
		{
			{if ((_x distance unconscious_body) < _follow_cam_distance && (side _x) == _ally_side_1 && alive _x &&  !(_x getVariable "NORRN_unconscious") || (_x distance unconscious_body) < _follow_cam_distance && (side _x) == _ally_side_2 && alive _x && !(_x getVariable "NORRN_unconscious")) then {_foo = _foo + [_x]}}forEach _friends;
		}else
		{
			{if (alive _x && (unconscious_body distance _x) <= _follow_cam_distance && !(_x getVariable "NORRN_unconscious")) then {_foo = _foo + [_x]}}forEach _NORRN_west;
		};
	...
	{	
		...
		_cam_hint6 = "";
		_rev = objNull;
		_rev = call compile format ["pr_norrn_dead_%1", player];
