...
{if (_x in _revive_units) then {call compile format ["_my_group = _my_group + ['%1']", _x]}}forEach (units _group);
...
// JIP respawns
if ((_JIP_respawns select 0) != 0) then 
{	
...
if (local _name) then
{
	...
	// Set current unit status
	call compile format ["pr_norrn_dead_%1 = objNull; publicVariable 'pr_norrn_dead_%1'", _name];
	...
	// Define smoke variable
	if (_AI_smoke == 1) then {call compile format["NORRN_dead_%1_smoke = false, publicVariable 'NORRN_dead_%1_smoke'", _unit]};
	
	call compile format ["if (%1 getVariable 'NORRN_unconscious') then
	{	
		_respawn = true; 
		_conscious = false;
		...
		_name setVariable ['NORRN_pos_x', (_pos select 0), true];	
		_name setVariable ['NORRN_pos_y', (_pos select 1), true];
		_name setVariable ['NORRN_unc_array', [_dir, _offset, _magazines, _weapons, _revive_damage], true];	
	};",_unit];
	
	// Get and set existing unconscious markers
	{call compile format ["if ((%1 getVariable 'NORRN_unconscious') && _unconscious_markers == 1 && isplayer _name) then 
	{	
		if (!isNull norrn_dead_%1) then
		{
			_mrker_pos = getPos norrn_dead_%1;
			_mrkr_ex = ""%1 is down"";
			createMarkerlocal [_mrkr_ex, _mrker_pos];
			_mrkr_ex setMarkerColorlocal ""ColorBlue"";
			_mrkr_ex setMarkerTypelocal ""Flag1"";
			_mrkr_ex setMarkerTextlocal ""%2 is down"";
			_mrkr_ex setMarkerSizelocal [0.4, 0.4];
		};
	}",_x, name _x];}forEach _revive_units;
...
{	
	...
	while{!local _name || isnull _name} do
	{	
...
// Unit alive
	...
	{	
		_name = call compile format ["%1", _name];
		_group = group _name;
		if (!local _name || isnull _name) exitWith {};
...
// Unit falls unconscious	
	...
	{	
		...
		_body setVariable ["NORRN_pos_x", (_pos select 0), true];	
		_body setVariable ["NORRN_pos_y", (_pos select 1), true];
		...
		// Anti-water code
		if (surfaceIsWater getPos _body && !_dead && _offset < 10) then	{_body = [_body, _unit] call Norrn_WaterFix};

		_pos = [(_body getVariable "NORRN_pos_x"), (_body getVariable "NORRN_pos_y"), 0];
		
		// Anti-slope code
		_slope = [_pos, 1] call GetTerrainSlope;
		if (_slope >= 0.78) then {[_pos, _slope] call Norrn_SlopeFix};
		...
		waitUntil{alive call compile format["%1",_unit]};
		_name = call compile format["%1",_unit];
		_name setVehicleInit "this switchMove 'AmovPpneMstpSnonWnonDnon_healed'; this allowDamage false"; 
		processInitCommands;
				
		_name setCaptive true;
		_name setdir _dir;
		_name setPos [(_pos select 0), (_pos select 1), _offset];
...
// While unconscious
	...
	{	
		...
		{	
			...
			if (_call_out_function == 1 && _d == 0 && !all_dead_check) then {_NORRN_random = [_name] call Norrn_CallOut};
			_d = _d + 1;
			...
			//AI smoke
			if (_AI_smoke == 1 && (_name getVariable "NORRN_AISmoke") select 0) then
			{
				((_name getVariable "NORRN_AISmoke") select 1) createVehicle _pos;
			};
			
			// Revive count-down
			if(_revive_timer == 1 && _visible_timer == 1 && _name == player) then
			{
				if (time > _timer) then
				{
					_timer = time + 1;
			...
			{	
				...
				{	
					...
					processInitCommands;
					if (_name == player) then {call compile format ["pr_norrn_dead_%1 = objNull; publicVariable 'pr_norrn_dead_%1'", _name];};
					_respawn = false;
					_conscious = false;
...
// Respawn at base
	...
	{	
		...
		{
			if (_drop_weapons == 1|| _cadaver == 1) then 
			{	
				sleep 2;
