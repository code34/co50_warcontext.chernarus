	...
	"wcbonuseventW" addPublicVariableEventHandler {
	...
	"wcmainmissionW" addPublicVariableEventHandler {
	...
	"wcconstructionkitpositionW" addPublicVariableEventHandler {
	...
	_position = [((getposasl WBASE_MOBILE) select 0),  ((getposasl WBASE_MOBILE) select 1) - 20, ((getposasl WBASE_MOBILE) select 2)];
	...
	WC_fnc_torespawnW = {
		...
		// wcbackpack = player addAction ["Backpack Weapon", "extern\x_scripts\x_backpack.sqf",[],-1,false];
		wccreatetrench = player addAction ["Create Trench", "warcontext\WC_fnc_createtrench.sqf",[],-1,false];
		//waituntil {(velocity player select 1) > 1.5};
		//nil = [] spawn WC_fnc_lifeslider;
	};
	...
	// introduction text
	PAPABEAR=[West,"HQ"]; PAPABEAR SideChat "Hi there. Your mission is to help us to win this WAR. You will receive your first instructions in few minutes. During this time take some ammos. Good luck soldiers";
