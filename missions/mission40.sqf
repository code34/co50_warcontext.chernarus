	...
	wcmissionname = "The Traitor";
	wcmissiondescriptionW = "One enemy colonel want to transmit us important informations, go to take him";
	wcmissiondescriptionE = "There is a Traitor colonel that want transmits some informations to US, kill him";
	wcmissiontarget = "Traitor";
	_missionnumber = 40;
	...
	while { !_missionend } do {
		if(wcobject) then {
			...
			deletevehicle _trg;
			_unit = _group createUnit ["RU_Commander", [(_position select 0) - 20, _position select 1], [], 0, "NONE"];
		};
		if (_unit distance WBASE_MOBILE < 100) then {
			nil = [nil,nil,rHINT,'Side mission: West wins ! The traitor is safe'] call RE;
		...
		if (!alive _unit) then {
			nil = [nil,nil,rHINT,'Side mission: East wins ! The traitor was killed'] call RE;
